...
stage=$1
data=exp/processed_debug

expname=resnet34_aamsoftmax_epoch30_cpu
dir=exp/$expname

mkdir -p $dir
mkdir -p $dir/pretrained

num_spk=`cat $data/num_spk`
echo "There are "$num_spk" number of speakers."

# feature prepare in kaldi
if [ $stage -le 6 ]; then
  ./feature_pre.sh $data $stage
fi
:<<!
# Network Training
if [ $stage -le 7 ]; then
  ...
  $cuda_cmd $dir/log/pretrain.log \
      ...
      --train-list $data/train_orig.scp --cv-list $data/cv_orig.scp \
      --spk-num $num_spk --utt2spkid $data/utt2spkid \
...
if [ $stage -le 8 ]; then
  ...
  $cuda_cmd $dir/log/train.log \
      python scripts/train_resnet_cpu.py \
      --gpu-num 0 --workers 2 \
...
if [ $stage -le 9 ]; then
  ...
  $cuda_cmd $dir/log/decode_${x}.log \
      ...
      --gpu-num 0 --workers 2 \
      --batch-size 1 --chunk-size -1 \
      --spk_num $num_spk --arch 'resnet34' \
      --input-dim 40 --pooling 'mean+std' \
      --model-path $model --decode-scp $data/decode_${x}.scp \

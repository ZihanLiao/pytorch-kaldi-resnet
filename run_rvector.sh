...
expname=resnet34_softmax_epoch30 # chance the experiment name to your liking
dir=exp/$expname
...
if [ $stage -le 7 ]; then
  ...
  $cuda_cmd $dir/log/train.log \
      ...
      --batch-size 1024 --print-freq 100 \
...
if [ $stage -le 12 ]; then
  ./test.sh $backend_log $backend_log plda $stage
  ./test.sh $backend_log $backend_log cosine $stage
fi
